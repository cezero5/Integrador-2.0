...
class ProductosProvider {
  ...
  /* Future<List<ProductoModel>> _procesarRespuesta(Uri url) async {
    final resp = await http.get(url);
    final decodedData = json.decode(resp.body);
    final producto = new ProductoModel.fromJson(decodedData['results']);

    return
  }*/

  Future<List<ProductoModel>> buscarProducto(String query) async {
    String _url = 'https://flutter-varios-d2538.firebaseio.com/productos';
    final url = Uri.https(_url, 'AIzaSyDL4dolvRlp9ZNz3kkYg3za2Uyvc4I_hlY');

    final resp = await http.get(url);
    final decodedData = json.decode(resp.body);
    final producto = decodedData;
